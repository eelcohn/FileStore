...
gpioWrite(TMPRW, PI_HIGH);
	...
	int transmitData(econet::Frame *frame, unsigned int length) {
		...
		do {
			...
			rpi_gpio::writeRegister(2, frame->data[i]);
			i++;
		} while (i < length);
...
//			gpioSleep(PI_TIME_RELATIVE, seconds, 0);	// Count pulses for 3 seconds
	...
	void getClockSpeed_inthandler(__attribute__((__unused__))int gpio, __attribute__((__unused__))int level, __attribute__((__unused__))unsigned int tick) {
		numpulses++;
	}
